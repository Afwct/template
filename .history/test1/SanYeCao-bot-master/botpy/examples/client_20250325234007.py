...
class MyClient(botpy.Client):
    ...
    async def on_group_at_message_create(self, message: GroupMessage):
        ...
        if msg == f"我喜欢你":
            ...
            flag = 0
            file_url = ""
            # D:/pythonProject/JMComic/18comic_dow/books/00003.jpg
            # TODO: 重复文件上传有问题
            if a is not None:
                flag = githubUpload.upload(
                    f"D:/pythonProject/JMComic/18comic_dow/books/{a}.pdf"
                )
            else:
                messageResult1 = await message._api.post_group_message(
                    group_openid=message.group_openid,
                    msg_type=0,
                    msg_id=message.id,
                ...
                return
            if flag is not 0:
                file_url = githubUpload.getImage(f"{a}.pdf")
            else:
                messageResult2 = await message._api.post_group_message(
                    group_openid=message.group_openid,
                    msg_type=0,
                    msg_id=message.id,
                    content=f"文件上传失败",
                ...
                return
            if file_url is None:
                messageResult3 = await message._api.post_group_message(
                    group_openid=message.group_openid,
                    msg_type=0,
                    msg_id=message.id,
                    content=f"文件获取失败",
