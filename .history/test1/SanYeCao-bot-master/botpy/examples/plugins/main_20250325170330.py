...
def autoDown(args):
    # 自定义设置：
    config = "D:/pythonProject/JMComic/config.yml"
    loadConfig = jmcomic.JmOption.from_file(config)
    # 如果需要下载，则取消以下注释
    # manhua = [args]
    # for id in manhua:
    # jmcomic.download_album(args, loadConfig)
    jmcomic.JmOption.default().to_file('./option.yml')
    # os.system('jmcomic %s' % args)
    # with open(config, "r", encoding="utf8") as f:
    #     data = yaml.load(f, Loader=yaml.FullLoader)
    #     path = data["dir_rule"]["base_dir"]

    with os.scandir("D:/pythonProject/JMComic/18comic_dow/books") as entries:
        for entry in entries:
            if entry.is_dir():
                if os.path.exists(os.path.join("D:/pythonProject/JMComic/18comic_dow/books/" + entry.name + ".pdf")):
                    print("文件：《%s》 已存在，跳过" % entry.name)
                    continue
                else:
                    print("开始转换：%s " % entry.name)
                    all2PDF("D:/pythonProject/JMComic/18comic_dow/books/" + entry.name, "D:/pythonProject/JMComic/18comic_dow/books", entry.name)
    return 1
