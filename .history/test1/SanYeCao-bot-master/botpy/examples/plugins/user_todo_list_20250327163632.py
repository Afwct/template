...
def show(user_name):
    cwd = Path.cwd()
    base_dir = cwd/Path("./test1/SanYeCao-bot-master/botpy/examples/user_data")
    # files = [os.path.join(base_dir, file) for file in os.listdir(base_dir)]
    flag = False
    json_data = None, todo_list = []
    # print(files)
    for file in base_dir.iterdir():
        # if (file/Path(f"{user_name}.json")).is_file():
        if file.is_file():
            flag = True
    if flag:
        if user_name == file.name:
...
def insert(user_name, todo):
    ...
    for file in base_dir.iterdir():
        print(f"**{file.name}**")
        if file.name == f'{user_name}.json':
            flag = True
            break
    if flag:
        if (file/Path(f"{user_name}.json")).is_file():
            with open(file/Path(f"{user_name}.json"), 'r') as file:
            ...
            with open(file/Path(f"{user_name}.json"), 'w') as file:
...
def file_not_found(base_dir, user_name):
    ...
    json_data = {'ToDo': ['还没有待办qwq', ' ']}
    with open(base_dir/Path(f"{user_name}.json"), 'w') as file:
        json.dump(json_data, file)
    not_found = -1
    return not_found
